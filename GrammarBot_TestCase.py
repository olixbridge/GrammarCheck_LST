...
for j in range(0,len(good_sentences)):
    ...
    if(text.find(" she ") != -1 and text.find("?")==-1 and text.find("'")==-1):
...
for j in range (1,100):
    try: 
        ...
        res = client.check(wrong_sentences[j]) # GrammarBotApiResponse(matches=[GrammarBotMatch(offset=2, length=4, rule={'CANT'}, category={'TYPOS'}), GrammarBotMatch(offset=26, length=5, rule={'CONFUSION_RULE'}, category={'TYPOS'})])
